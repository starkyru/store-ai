...
```typescript
import { createChatManager } from '@store-ai/core';

const manager = createChatManager({
  provider: anthropic(),
  middleware: [persist(indexedDBAdapter('my-app'))],
});
...
manager.subscribe((chats) => {
  console.log(chats); // [{ id: 'chat-1', messageCount: 5, ... }, ...]
});

// Each chat has independent state
chat1.submit({ message: 'Hello' });
chat2.submit({ message: 'Hi there' });

// Get active store for framework binding
const activeStore = manager.active; // AIStore for chat-1
```
...
Create a multi-chat coordinator.
...
function createChatManager(options?: ChatManagerOptions): ChatManager;
```
...
| ------------------------ | ---------------- | ------------------------------------ |
| `logging(opts?)`         | `@store-ai/core` | Log stream events to console         |
| `validateSchema(schema)` | `@store-ai/core` | Zod validation for structured output |
| `throttle(ms)`           | `@store-ai/core` | Throttle state updates               |
| `persist(storage)`       | `@store-ai/core` | Persist messages to storage          |
| `retryOn(opts)`          | `@store-ai/core` | Auto-retry on transient errors       |
| `trackCost(pricing)`     | `@store-ai/core` | Track token costs                    |
| `mapEvents(fn)`          | `@store-ai/core` | Transform stream events              |
...
| `passthrough()`     | Raw text (no SSE framing)                               |
